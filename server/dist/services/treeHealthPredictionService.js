...
import { HealthTrend } from '../types/treeHealth.js';
...
class TreeHealthPredictionService {
     ...
     * @returns 健康状态预测
     */
    async predictHealthStatus(task, tree) {
     ...
     */
    calculateRiskLevel(task, tree) {
        if (task.status === 'COMPLETED')
            return 0;
        if (!task.dueDate)
            return 30; // 无截止日期默认中等风险
        const now = new Date();
        const deadline = new Date(task.dueDate);
        // 已过期任务最高风险
        if (now > deadline)
...
//# sourceMappingURL=treeHealthPredictionService.js.map