...
class TaskTreeService {
     ...
     * @returns 创建的树木对象
     */
    async createTreeForTask(taskId, treeType) {
        try {
            // 先检查任务是否存在
     ...
     * @returns 更新后的树木
     */
    async updateTreeOnTaskChange(taskId, status, progress) {
        try {
            const tree = await prisma.tree.findFirst({
                where: { taskId: taskId }
            });
...
//# sourceMappingURL=taskTreeService.js.map