...
export async function getTrees(req, res) {
    try {
        // 获取查询参数
        const { type, stage, taskId, page = '1', pageSize = '10', sortBy = 'createdAt', sortOrder = 'desc' } = req.query;
        // 构建过滤条件
        const where = {};
        if (type) {
...
export async function createTree(req, res) {
    try {
        const { type, taskId, stage = 1, healthState = 100, position } = req.body;
        if (!type) {
            return res.status(400).json({
                code: 400,
                data: null,
 ...
 */
export async function bindTaskToTree(req, res) {
    try {
 ...
 */
export async function unbindTaskFromTree(req, res) {
    try {
        const { id } = req.params;
        if (!id) {
            return res.status(400).json({
