...
async function testTaskAnalysis() {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error('❌ 任务分析API测试失败:', (error as Error).message);
    }
    return null;
  }
}
...
// 测试任务拆解API
async function testTaskDecomposition(analysisResult: any) {
  console.log('\n----- 测试任务拆解API -----');
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      console.error('❌ 任务拆解API测试失败:', (error as Error).message);
    }
  }
}
