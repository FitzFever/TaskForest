...
export class AIController {
   ...
   * 分析任务，提供复杂度和时间预估
   */
  async analyzeTask(req: Request, res: Response): Promise<void> {
    try {
   ...
   * 拆解任务，生成子任务列表
   */
  async breakdownTask(req: Request, res: Response): Promise<void> {
    try {
      const taskInfo: TaskInfo = req.body;
   ...
   * 分析并拆解任务，返回完整结果
   */
  async analyzeAndBreakdownTask(req: Request, res: Response): Promise<void> {
    try {
      const taskInfo: TaskInfo = req.body;
   ...
   * 拆解并创建任务，自动创建主任务和子任务
   */
  async breakdownAndCreateTasks(req: Request, res: Response): Promise<void> {
    try {
   ...
   * 验证任务信息是否完整
   */
  private validateTaskInfo(taskInfo: TaskInfo): boolean {
