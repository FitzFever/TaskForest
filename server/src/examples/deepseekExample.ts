...
import { TaskAnalysisRequest, TaskDecompositionRequest, TaskComplexity } from '../types/DeepSeekAPI.js';
  ...
  try {
    ...
    try {
      ...
      if (analysisResult.complexity) {
        const decompositionRequest: TaskDecompositionRequest = {
        ...
        try {
          ...
          console.error('任务拆解失败:', (error as Error).message);
        }
      }
    } catch (error) {
      console.error('任务分析失败:', (error as Error).message);
    }
    
    // 示例3: 分析复杂任务
    const complexTaskRequest: TaskAnalysisRequest = {
      taskId: 'task-456',
      title: '开发完整的电子商务平台',
    ...
    try {
      ...
      if (complexResult.complexity) {
        const complexDecompositionRequest: TaskDecompositionRequest = {
        ...
        try {
          ...
          console.error('复杂任务拆解失败:', (error as Error).message);
        }
      }
    } catch (error) {
      console.error('复杂任务分析失败:', (error as Error).message);
    }
    
    console.log('\nDeepSeek服务示例完成');
  } catch (error) {
    logger.error('DeepSeek示例运行失败', { error });
    console.error('示例运行失败:', (error as Error).message);
  }
}
