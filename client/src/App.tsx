...
const AITaskGenerator = React.lazy(() => import('./pages/AITaskGenerator'));
const TextToTask = React.lazy(() => import('./pages/TextToTask'));
      ...
      <Header style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <Menu
          ...
          items={[
            ...
            { key: 'ai-task', icon: <RobotOutlined />, label: <Link to="/ai-task">AI任务生成</Link> },
            { key: 'text-to-task', icon: <FileTextOutlined />, label: <Link to="/text-to-task">文本转任务</Link> },
      ...
      <Content style={{ padding: '20px', background: '#f0f2f5' }}>
        <React.Suspense fallback={<Spin size="large" style={{ margin: '20px auto', display: 'block' }} />}>
          <Routes>
            ...
            <Route path="/ai-task" element={<AITaskGenerator />} />
            <Route path="/text-to-task" element={<TextToTask />} />
